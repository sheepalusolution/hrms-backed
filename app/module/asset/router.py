...
# Optional: Assign an asset to an employee
@router.put("/{asset_id}/assign", response_model=schemas.AssetOut)
def assign_asset(asset_id: int, employee_id: int, db: Session = Depends(get_db)):
    asset = db.query(models.Assets).filter(models.Assets.id == asset_id).first()
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")

    asset.employee_id = employee_id
    asset.status = schemas.AssetStatusEnum.assigned
    db.commit()
    db.refresh(asset)
    return asset
