...
# -----------------------------
@router.post(
    "", response_model=schemas.DesignationOut, status_code=status.HTTP_201_CREATED
)
def create_designation(
    designation: schemas.DesignationCreate, db: Session = Depends(get_db)
):
...
def get_designation(designation_id: int, db: Session = Depends(get_db)):
    designation = (
        ...
        .filter(models.Designation.id == designation_id)
        .first()
    )
    if not designation:
...
def update_designation(
    designation_id: int,
    designation_data: schemas.DesignationUpdate,
